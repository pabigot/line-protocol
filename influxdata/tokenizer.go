...
func (t *Tokenizer) advanceTagComma() error {
	...
	t.advance(1)
	if !t.ensure(1) {
		return t.syntaxErrorf("expected tag name after comma; got end of input")
	}
	if whitespace.get(t.at(0)) {
		return t.syntaxErrorf("expected tag name after comma; got white space instead")
	}
	return nil
}
...
func (t *Tokenizer) NextFieldBytes() (key []byte, kind ValueKind, value []byte, err error) {
	...
	if !fieldSeparatorSpace.get(nextc) {
		return nil, Unknown, nil, t.syntaxErrorf("unexpected character '%c' after field", nextc)
	}
	t.take(fieldSeparatorSpace)
	if !t.ensure(1) {
		return fieldKey, fieldKind, fieldVal, nil
